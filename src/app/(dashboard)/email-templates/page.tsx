    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">{displayTemplates.length} templates</p>
        </div>
          ...
          {displayTemplates.map((t) => (
            <div key={t.id} className="card p-5">
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                {t.variables && t.variables.slice(0, 4).map((v: string) => (
                  <span key={v} className="badge badge-gray text-xs">{`{{${v}}}`}</span>
                ))}
                {t.variables && t.variables.length > 4 && <span className="badge badge-gray text-xs">+{t.variables.length - 4}</span>}
              </div>
